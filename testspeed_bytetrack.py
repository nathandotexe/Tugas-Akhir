...
FLASK_PORT       = 5000
FLASK_ENABLED    = True   # set False to disable web stream
...
def load_calibration(calib_path):
    """
    Load undistortion maps from calibration.npz (output of camera_calib.py).
    Returns (map1, map2) for cv2.remap(), or (None, None) if unavailable.
    """
    if calib_path is None:
        print("  [Calib] CALIB_PATH is None — skipping undistortion.")
        return None, None
...
class FlaskStreamer:
    ...
    def push(self, frame):
        """Encode and store the latest annotated frame."""
    ...
    def _index(self):
        return (
            "<html><head><title>Athlete Speed Tracker</title>"
...
def open_capture(source):
    ...
    if is_rtsp:
        ...
        if cap.isOpened():
            ...
            return cap
        raise RuntimeError(
            f"Cannot open RTSP stream: {source}\n"
        )
...
def show_field_preview(frame, corners):
    ...
    cv2.polylines(preview, [poly], isClosed=True, color=(0,255,180), thickness=2)
...
def run_field_setup(frame, H):
    ...
    return corners, start_seg, finish_seg, mask
...
def make_kalman(fps, wx, wy):
    ...
    kf.Q = np.diag([0.05, 0.05, 0.5, 0.5])  # top-down: higher velocity noise for sprints
    return kf
...
class TrackState:
    __slots__ = ("kf","prev_t","prev_px","speed_buf","last_speed_ms",
                 "color","start_crossed","finish_crossed")

    def __init__(self, kf, t, px, py, color):
        self.kf             = kf
        self.prev_t         = t
        self.prev_px        = (px, py)
...
def generate_speed_graph(speed_logs, start_events, finish_events, out_path):
    if not speed_logs:
        print("No speed data."); return
    from scipy.ndimage import gaussian_filter1d
    n = len(speed_logs)
    fig, axes = plt.subplots(n, 1, figsize=(13, 3.8*n), squeeze=False)
    fig.suptitle("Athlete Speed Profiles", fontsize=16, fontweight="bold", y=1.01)
    ...
    for row, (tid, records) in zip(axes, sorted(speed_logs.items())):
        ...
        ax.plot(ts, kmph, color=col, alpha=0.2, linewidth=1)
        smooth = gaussian_filter1d(kmph, sigma=4)
        ax.plot(ts, smooth, color=col, linewidth=2.3, label=f"ID {tid} (km/h)")
        for st in start_events.get(tid, []):
            ax.axvline(st, color="lime", linestyle=":", linewidth=1.5, label="Start")
        for ft in finish_events.get(tid, []):
            ax.axvline(ft, color="red", linestyle="--", linewidth=1.5, label="Finish")
        pk = int(np.argmax(smooth))
        ax.annotate(f"Peak {smooth[pk]:.1f} km/h", xy=(ts[pk], smooth[pk]),
                    xytext=(ts[pk]+0.5, smooth[pk]+1.5), fontsize=8, color="darkred",
...
def main():
    ...
    first_frame = grab_first_frame(source)
    first_frame = undistort_frame(first_frame, calib_map1, calib_map2)

    setup = run_field_setup(first_frame, H)
    if setup is None:
        print("Field setup cancelled."); return
    corners, start_seg, finish_seg, roi_mask = setup
    ...
    while True:
        if paused:
            ...
            if ret and live is not None:
                ...
                if new_setup is not None:
                    corners, start_seg, finish_seg, roi_mask = new_setup
                    tracks.clear()
                    print("  Recalibrated.")
            paused = False
        ...
        frame     = undistort_frame(frame, calib_map1, calib_map2)  # lens correction
        frame_idx += 1
        ...
        dark         = annotated.copy()
        dark[outside_mask > 0] = (dark[outside_mask > 0] * 0.4).astype(np.uint8)
        cv2.addWeighted(dark, 0.5, annotated, 0.5, 0, annotated)
        cv2.polylines(annotated, [np.array(corners, dtype=np.int32)],
                      isClosed=True, color=(0,255,180), thickness=2, lineType=cv2.LINE_AA)
        ...
        if det.boxes is not None and det.boxes.id is not None:
            ...
            for box, tid, conf in zip(boxes, ids, confs):
                x1, y1, x2, y2 = box
                cx_px = (x1 + x2) * 0.5
                cy_px = (y1 + y2) * 0.5  # top-down: center is true ground position
                ...
                if tid not in tracks:
                    tracks[tid] = TrackState(
                        kf=make_kalman(vid_fps, wx, wy), t=t_now,
                        px=int(cx_px), py=int(cy_px), color=rand_color(tid)
                    )
                ...
                kf.Q = np.diag([0.05, 0.05, 0.5*q_scale, 0.5*q_scale])  # top-down adaptive Q
                kf.predict()
                kf.update([wx, wy])

                speed_ms = np.hypot(float(kf.x[2]), float(kf.x[3]))
                ...
                if frame_idx % LOG_EVERY_N == 0:
                    speed_logs[tid].append((t_now, disp_ms))

                col = state.color
                cv2.rectangle(annotated, (int(x1),int(y1)), (int(x2),int(y2)), col, 2)

                label = f"ID {tid}  {disp_ms:.1f}m/s ({disp_kh:.1f}km/h)"
        ...
        cv2.line(annotated, start_seg[0],  start_seg[1],  (0,255,0),  2, cv2.LINE_AA)
        cv2.line(annotated, finish_seg[0], finish_seg[1], (0,80,255), 2, cv2.LINE_AA)
        cv2.putText(annotated, "START",  start_seg[0], cv2.FONT_HERSHEY_SIMPLEX,
                    0.65, (0,255,0),  2, cv2.LINE_AA)
        cv2.putText(annotated, "FINISH", finish_seg[0], cv2.FONT_HERSHEY_SIMPLEX,
                    0.65, (0,80,255), 2, cv2.LINE_AA)
        ...
        if flask_streamer: flask_streamer.push(annotated)  # push to web stream
    ...
    for tid in sorted(speed_logs):
        ...
        print(f"  ID {tid:3d}  Peak:{max(spds)*3.6:6.2f}km/h  "
              ...
              f"Finishes:{len(finish_events[tid])}  Splits:[{t_str}]")

    generate_speed_graph(speed_logs, start_events, finish_events, GRAPH_OUTPUT)
